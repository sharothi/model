...
from .resources import QuizResource, OptionResource
    ...
    if request.method == "POST":
        ...
        if quiz_form.is_valid():
            ...
            try:
                ...
                return render(request, 'teacher/questions_edit_form.html' , { 'quiz_form':quiz_form, 'message': 'Please input your data correctly' })

            else:
                quiz_form = QuizForm()
                return render(request, 'teacher/questions_edit_form.html' , { 'quiz_form':quiz_form, 'message': 'Submited' ,'number_of_questions':number_of_questions })
        
    context = {'quiz_form':quiz_form, 'options':options}
    return render(request, 'teacher/questions_edit_form.html', context)
...
def std_info_institu(request):
    ...
    # print(students)
    return render(request, 'teacher/stdinfo_institu.html', {'students':students})
...
def exam_list_view(request):
    ...
    if exam_list:
        if request.user.student.approved:
            ...
            exam_list = ExaminationInfo.objects.filter(published=True, pay_status='free')
            user_exam_done = None
            exams=[]
            print(exam_list)
            for exam in exam_list:
...
def get_paid_account(request):
    ...
    # return render(request, 'student/exam_view1.html', {'message':'There are no Qusetions available','activation_message':activation_message})
...
def quiz_view(request, id , pk=0):
    ...
    if request.method == 'POST' :
        ...
        que_data = {'title':str(quiz),'client_given':client_given[1:],'status':True}
...
#@@@@ ========================== DB import export views =====================================@@@

QuizResource, OptionResource

def export_data(request):
    quiz_resource = QuizResource()
    option_resource = OptionResource()

    dataset = option_resource.export()
    response = HttpResponse(dataset.csv, content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="options.csv"'
    return response
...
def import_data(request):
    if request.method == 'POST':
        ...
        imported_data = dataset.load(new_options.read().decode('utf-8'), format='csv')
        result = option_resource.import_data(dataset, dry_run=True)  # Test the data import
        if not result.has_errors():
            option_resource.import_data(dataset, dry_run=False)  # Import the actual data
